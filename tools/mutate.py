...
MECH_MODES = [
    ...
    "mech-ghost-kills", "mech-cascade", "mech-aim", "mech-jump", "mech-air",
...
MUTANTS = [
    ("bubble-never-rises", "a bubble that just hangs where it stops",
     "      if (b.y > apex) b.y -= BUB_RISE * (b.holds ? 0.5 : 1) * dt;", "      if (false) b.y -= 0;"),
    ...
    ("captives-never-escape", "a bubbled monster is bubbled forever",
     "    if (b.age > (b.holds && b.holds.angry && !G.calm ? 4.5 : BUB_LIFE)) {", "    if (b.age > 1e9) {"),
    ...
    ("anger-is-cosmetic", "angry monsters move at normal speed",
     "  const rageMul = G.calm ? (e.angry ? 1.45 : 1) : (e.rage >= 2 ? RAGE2 : (e.angry ? ESCAPE_ANGRY : 1));", "  const rageMul = 1;"),
    ...
    ("ghost-does-not-hunt", "the hurry-up ghost drifts aimlessly",
     "    G.ghost.vx += Math.sign(tx - G.ghost.x) * 250 * dt;", "    G.ghost.vx += 0 * dt;"),
    ...
    ("monsters-never-leave-a-ledge", "the keep's monsters never come down",
     "      if ((G.calm || !e.angry) && !(p.alive && p.y > e.y + TILE)) e.vx = -e.vx;", "      e.vx = -e.vx;"),
    ("bubble-is-born-poppable", "the dragon eats its own breath",
     ...
     "    if (false) continue;"),
    ("roof-is-passthrough", "the dragon can jump through the roof and strand itself",
     "    if ((!dropThrough || head < TILE + 2) && (solidPx(e.x - halfW + 4, head) || solidPx(e.x + halfW - 4, head))) {",
     "    if ((!dropThrough) && (solidPx(e.x - halfW + 4, head) || solidPx(e.x + halfW - 4, head))) {"),
    ("air-is-free", "breath costs nothing",
     "  p.air -= 1;", "  p.air -= 0;"),
    ("no-air-refund", "a catch no longer refunds a breath",
     "          if (G.player) G.player.air = Math.min(AIR_MAX, (G.player.air || 0) + 1);  // a hit is free breath",
     "          ;"),
    ("no-air-regen", "lungs never refill",
    ...
    ("chain-window-forever", "one endless chain: every kill after the first pays 8000",
     "CHAIN_WINDOW = 1.6, ESCAPE_ANGRY = 1.75", "CHAIN_WINDOW = 600, ESCAPE_ANGRY = 1.75"),
